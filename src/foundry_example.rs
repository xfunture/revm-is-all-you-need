...
use ethers::{
    ...
    plelude::*,
    provider::Middleware,
    types::{BlockNumber, DefaultFrame, H160, U256}, utils::keccak256,
};
...
pub async fn foundry_v2_simulate_swap<M:Middleware + 'static>(
    ...
    // SharedBackend, which is a way of forking the mainnet in Foundry.
    // This backend system will automatically retrieve all the account information
    // and storage values for us from behind the scenes whenever we try to access values that have not been called before.
    ...
    let simulator_address = H160::from_str("0xF2d01Ee818509a9540d8324a5bA52329af27D19E");
    ...
    ork_db.insert_account_storage(input_token.into(), balance_slot.into(), ten_eth)?;
    

    // run v2SimulateSwap
    let amount_in = U256::from(1).checked_mul(U256::from(0).pow(U256::from(18))).unwrap();
